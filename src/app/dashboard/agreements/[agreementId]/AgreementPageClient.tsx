...
export default function AgreementPageClient({ agreementId }: { agreementId: string }) {
  ...
  useEffect(() => {
    ...
    function selectSigner(signerId: string) {
        const s = allSignersArray().find(x => x.id === signerId);
        if (!s) return;
        state.selectedSigner = s.id;
        if (signerLabel) signerLabel.textContent = `${s.name}`;
        if (signerAvatar) signerAvatar.textContent = s.name.split(' ').map((p:string) => p[0]).slice(0,2).join('').toUpperCase();
        if (signerMenu) signerMenu.classList.add('hidden');
        refreshPrimaryBtn();
        updateProgress();
    }
    
    // Auto-select signer if the current user is a participant
    if (userProfile?.email) {
        const currentUserSigner = allSignersArray().find(s => s.email.toLowerCase() === userProfile.email!.toLowerCase());
        if (currentUserSigner) {
            selectSigner(currentUserSigner.id);
        }
    }
