...
import { getAgreementsAction } from '@/actions/agreementActions';
...
export default function AgreementsPage() {
    ...
    useEffect(() => {
        async function loadData() {
            setIsLoading(true);
            const result = await getAgreementsAction();
            if (result.status === 'success' && result.data) {
                // Assuming the data from Firestore matches the Contract type for now
                // This might need a mapping function if the structure is different
                const firestoreContracts: Contract[] = result.data.map(item => ({
                    id: item.id,
                    title: item.songTitle || 'Untitled Agreement',
                    tags: item.tags?.join(', ') || 'music, contract',
                    category: item.category || 'general',
                    type: item.type || 'Contrato',
                    status: item.status as Contract['status'],
                    mins: '5', // Placeholder
                    filetypes: 'PDF', // Placeholder
                    verified: true, // Placeholder
                    image: item.image || `https://placehold.co/400x225.png`,
                    desc: item.description || 'No description available.',
                    shortDesc: item.shortDescription || 'No short description.',
                }));

                const deletedIds: string[] = JSON.parse(localStorage.getItem(DELETED_CONTRACTS_KEY) || '[]');
                const activeContracts = firestoreContracts.filter((c: Contract) => !deletedIds.includes(c.id));
                setContractData(activeContracts);
            } else {
                console.error("Failed to load agreements:", result.message);
                ...
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {Array.from({ length: 8 }).map((_, i) => (
                        <div key={i} className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4">
                            <div className="animate-pulse bg-slate-700/50 h-32 rounded-lg"></div>
                            <div className="space-y-2">
                                <div className="animate-pulse bg-slate-700/50 h-6 w-3/4 rounded"></div>
                                <div className="animate-pulse bg-slate-700/50 h-4 w-full rounded"></div>
                                <div className="animate-pulse bg-slate-700/50 h-4 w-1/2 rounded"></div>
                            </div>
                        </div>
                    ))}
                </div>
